...
void StudentFlowController::showMainMenu() {
  vector<string> menu = {"Register in Course", "List My Courses", "View Course", "Grades Report",
                         "Log out"};
...
void StudentFlowController::showCourseMenu() {
  std::vector<std::string> menu = {"Unregister from course ", "Submit solution", "Back"};
...
void StudentFlowController::unregisterFromCourse() {
  Helper::removeElement(this->currentCourse->registeredStudents, this->currentStudent);
  Helper::removeElement(this->currentStudent->registeredCourses, this->currentCourse);
}

void StudentFlowController::submitCourseAssignment() {
  int assignmentsCount = (int)this->currentCourse->assignments.size();

  std::cout << "\nWhich ith [1 - " << assignmentsCount << " ] assignment to submit? ";

  int choice = Helper::readInt(1, assignmentsCount) - 1;

  std::shared_ptr<Assignment> currentAssignment = this->currentCourse->assignments[choice];

  std::shared_ptr<AssignmentSolution> newAssignmentSolution(new AssignmentSolution());

  std::cout << "Enter answer: ";
  std::getline(std::cin, newAssignmentSolution->answer);
  ...
  std::vector<std::shared_ptr<AssignmentSolution>>& studentAssignmentsSolutions =
      this->currentStudent->assignmentSolutions;
  ...
  for (int i = 0; i < (int)studentAssignmentsSolutions.size(); i++) {
    if (studentAssignmentsSolutions[i]->assignment == currentAssignment) {
      std::vector<std::shared_ptr<AssignmentSolution>>& currentCourseAssignmentSolutions =
          currentAssignment->assignmentSolutions;

      for (int j = 0; j < (int)currentCourseAssignmentSolutions.size(); j++) {
        if (studentAssignmentsSolutions[i] == currentCourseAssignmentSolutions[j]) {
          currentCourseAssignmentSolutions.erase(currentCourseAssignmentSolutions.begin() + j);
          break;
        }
      }

      studentAssignmentsSolutions.erase(studentAssignmentsSolutions.begin() + i);
      break;
    }
  }
