...
std::shared_ptr<AssignmentSolution> Student::getAssignmentSolution(
  ...
  for (std::shared_ptr<AssignmentSolution> assignmentSolution :
...
Student::computeGradesStatistic() {
  ...
  for (std::shared_ptr<AssignmentSolution> studentAssignmentSolution :
       this->assignmentSolutions) {
    std::string courseCode =
        studentAssignmentSolution->assignment->course->code;

    if (studentAssignmentSolution->isGraded) {
      courseCodeToTotalStudentGradesSum[courseCode] +=
          studentAssignmentSolution->grade;
    }
    courseCodeToTotalGradesSum[courseCode] +=
        studentAssignmentSolution->assignment->maxGrade;
    courseCodeToTotalAssignments[courseCode]++;
  }

  for (std::pair<std::string, int> studentCourseGrades :
       courseCodeToTotalStudentGradesSum) {
